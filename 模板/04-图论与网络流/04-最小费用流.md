...
| 操作 | 含义 | 复杂度 | 备注 |
|---|---|---|---|
| 构造 | `MinCostFlow<T>(n)` 构造含 `n` 个点的空网络 | 时间、空间 $O(n)$ | 点编号从 `0` 开始，也可默认构造后自动扩点 |
| 清空 | `init(n)` 清空原图并重新建立 `n` 个点 | 时间、空间 $O(n)$ | 已保存的边编号失效 |
| 加边 | `addEdge(u,v,cap,cost)` 添加容量与单位费用，返回正向残量边编号 | 摊还 $O(1)$ | `cap` 为 `int`，`cost` 为 `T`；平行边分别保留 |
| 增广 | `flow(s,t,limit)` 返回增加的流量与费用 | $O(VE+AE\log V)$ | $A$ 为增广次数；省略 `limit` 时求最小费用最大流 |
| 查询边 | `getEdge(i)` 返回原边端点、容量、费用与当前流量 | $O(1)$ | `i` 为 `addEdge` 的返回值 |
| 修改边 | `changeEdge(i,c,f)` 将边改为容量 `c`、流量 `f` | $O(1)$ | 要求 $0\le f\le c$，单位费用不变 |

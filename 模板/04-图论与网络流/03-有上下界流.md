...
| 操作 | 含义 | 复杂度 | 备注 |
|---|---|---|---|
| 构造 | `BoundedFlow(n)` 构造含 `n` 个点的网络 | 时间、空间 $O(n)$ | 点编号为 `0..n-1` |
| 加边 | `addEdge(u,v,l,r)` 添加流量下界为 `l`、上界为 `r` 的有向边 | $O(1)$ | 要求 $0\le l\le r$，边按加入顺序编号 |
| 可行流 | `feasible()` 判断是否存在循环可行流 | 时间 $O(V^2E)$，空间 $O(V+E)$ | 成功时 `ans[i]` 为第 `i` 条边的流量 |
| 流量范围 | `flowRange(s,t)` 返回可行性及最小、最大流量 | 时间 $O(V^2E)$，空间 $O(V+E)$ | 只考虑非负流量；对应方案保存在 `minAns`、`maxAns` |

...
>
> **使用条件：** 整数坐标为 `i64` 时，加减、数乘、平方和、点积与叉积的中间值必须在 `i64` 范围内，否则相应运算改为 `i128`；浮点比较需按题目精度要求自行处理。

| 接口 | 含义 | 复杂度 | 备注 |
|---|---|---|---|
| `p*q`、`p&q` | 叉积、点积 | $O(1)$ | `i64` 需保证乘法及加减的中间值不溢出 |
| `p*k`、`p/k` | 数乘、数除 | $O(1)$ | 除数不能为 $0$；`i64` 数乘不得溢出 |
| `p.dis2()`、`p.dis()` | 点到原点距离的平方、点到原点的距离；两点 `a`、`b` 的距离写作 `(a-b).dis()` | $O(1)$ | `i64` 需保证坐标差、平方及平方和不溢出 |
| `p.unit()` | 单位向量 | $O(1)$ | 只用于非零 `Point<double>` |
| `p.rotate90()` | 逆时针旋转 $90^\circ$ | $O(1)$ | `i64` 坐标取负不得溢出 |
| `p.rotate(a)` | 逆时针旋转 `a` 弧度 | $O(1)$ | 只用于 `Point<double>` |
| `convexHull(a)` | 求 Andrew 凸包 | 时间 $O(n\log n)$，空间 $O(n)$ | 按精确比较书写；浮点比较需自行处理，`i64` 叉积不得溢出 |

...
|---|---|---|
| `p*q`、`p&q` | 叉积、点积 | $O(1)$ |
| `p*k`、`p/k` | 数乘、数除，除数不能为 $0$ | $O(1)$ |
| `p.dis2()`、`p.dis()` | 点到原点距离的平方、点到原点的距离；两点 `a`、`b` 的距离写作 `(a-b).dis()` | $O(1)$ |
| `p.unit()` | 单位向量，只用于非零 `Point<double>` | $O(1)$ |
| `p.rotate90()` | 逆时针旋转 $90^\circ$ | $O(1)$ |
| `p.rotate(a)` | 逆时针旋转 `a` 弧度，只用于 `Point<double>` | $O(1)$ |
| `convexHull(a)` | 求 Andrew 凸包 | 时间 $O(n\log n)$，空间 $O(n)$ |

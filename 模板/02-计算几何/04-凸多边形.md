...
| 接口 | 含义 | 复杂度 |
|---|---|---|
| `pointConvexRelation(p, a)` | 点在凸多边形外部、边界、内部分别返回 $0,1,2$ | 时间 $O(\log n)$，空间 $O(1)$ |
| `convexDiameter2(a)` | 凸多边形直径的平方 | 时间 $O(n)$，空间 $O(1)$ |
| `minkowskiSum(a, b)` | 两个凸多边形的闵可夫斯基和，返回逆时针顶点序列 | 时间 $O(n+m)$，空间 $O(n+m)$ |

...
| 接口 | 含义 | 复杂度 |
|---|---|---|
| `l.angle()` | 有向直线的方向角 | $O(1)$ |
| `parallel(a, b)`、`perpendicular(a, b)` | 两直线平行、垂直 | $O(1)$ |
| `onLine(p, l)`、`onSegment(p, l)` | 点在直线、线段上，线段包含端点 | $O(1)$ |
| `crs(a, b)` | 两直线交点 | $O(1)$ |
| `project(p, l)` | 点在直线上的投影点 | $O(1)$ |
| `disLine(p, l)`、`disSegment(p, l)` | 点到直线、线段的距离 | $O(1)$ |
| `segmentIntersection(a, b)` | 两线段交集类型：$0$ 不交，$1$ 严格相交，$2$ 单点接触，$3$ 共线重叠 | $O(1)$ |

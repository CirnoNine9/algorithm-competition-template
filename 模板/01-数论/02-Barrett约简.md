...
**常数参考（中位数）：** 对固定的正负混合随机 `i64` 输入重复执行运行时取模，$mod=10^9+7$、$x=10^8$ 次时，原生 `a % mod` 约 $0.200\,\mathrm{s}$，当前 `Barrett` 约 $0.088\,\mathrm{s}$；$x=5\times10^8$ 次时分别约 $1.000\,\mathrm{s}$、$0.439\,\mathrm{s}$。

...
const categoryDescriptions: Record<string, string> = {
  ...
  '03-数据结构': '树状数组、平衡树、树链剖分、动态树、重心分解与字符串自动机',
...
function renderHomepage(
  ...
  return [
    ...
    '  <div><strong>70</strong><span>独立模板</span></div>',
...
async function main(): Promise<void> {
  ...
  if (documents.length !== 70) throw new Error(`Expected 70 template documents, found ${documents.length}.`);
  ...
  if (htmlFiles.length !== 78) {
    throw new Error(`Expected 78 HTML pages, generated ${htmlFiles.length}.`);
  }
